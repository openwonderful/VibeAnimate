...
const url = process.argv[2] || 'http://localhost:5173/?act=7'
const out = process.argv[3] || 'out/screenshots/act7_iter3.png'
const sleep = ms => new Promise(r => setTimeout(r, ms))

// Launch Chrome directly with the target URL (not about:blank)
const chrome = spawn('google-chrome', [
  '--headless=new', '--enable-unsafe-swiftshader', '--enable-webgl',
  '--use-angle=swiftshader', '--disable-dev-shm-usage',
  '--remote-debugging-port=9229', '--window-size=1920,1080',
  '--no-sandbox', url
], { stdio: 'pipe' })

async function tryConnect(port, retries = 30) {
...
try {
  ...
  const tab = await tryConnect(9229)
  ...
  // Extra render time
  console.log('Waiting 4s for GL render...')
  await sleep(4000)

  // Try canvas toDataURL (since preserveDrawingBuffer is true)
  ...
  if (val && val.startsWith('data:image')) {
    ...
    console.log(`Saved canvas capture: ${buf.length} bytes`)
  } else {
    ...
    try {
      ...
      if (ss?.data) {
        ...
        writeFileSync(out, buf)
        console.log(`Saved page screenshot: ${buf.length} bytes`)
      }
    } catch (e2) {
