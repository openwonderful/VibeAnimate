/**
 * Screenshot helper that appends ?nopp=1 to the URL so the scene
 * can skip heavy postprocessing (EffectComposer) for headless capture.
 */
...
const port = portArg ? parseInt(portArg.split('=')[1]) : 9238
const positional = args.filter(a => !a.startsWith('--'))
let url = positional[0] || 'http://localhost:5173/?act=char-human-a'
// Append nopp flag
url += (url.includes('?') ? '&' : '?') + 'nopp=1'
const out = positional[1] || 'out/screenshots/char-nopp.png'
...
try {
  ...
  const glInfo = await cdp('Runtime.evaluate', {
    expression: `(() => {
      ...
      if (!gl) return 'no gl on existing canvas';
  ...
  if (val && val.startsWith('data:image')) {
    ...
    console.log(`Canvas capture: ${buf.length} bytes -> ${out}`)
  } else {
    ...
    if (ss?.data) {
      ...
      writeFileSync(out, buf)
      console.log(`Page screenshot: ${buf.length} bytes -> ${out}`)
    }
  }
