/**
 * KuwaharaEffect — anisotropic Kuwahara painterly/watercolor filter.
 *
 * Vendored verbatim from @tresjs/post-processing v3.7.2 (MIT licensed).
 * Source: https://github.com/Tresjs/post-processing/blob/main/src/core/pmndrs/custom/kuwahara/index.ts
 *
 * Extends the pmndrs/postprocessing `Effect` base class so it composes with
 * @react-three/postprocessing's `<EffectComposer>` like any first-party effect.
 *
 * The shader treats the input buffer as both color source and as a proxy
 * structure tensor (RGB → Jxx, Jyy, Jxy). This is a simplification of the
 * proper anisotropic Kuwahara (which usually wants a precomputed structure
 * tensor pre-pass) and produces a watercolor-like result in a single pass.
 */
...
void getSectorVarianceAndAverageColor(mat2 anisotropyMat, float angle, float radius, out vec3 avgColor, out float variance) {
    ...
    float eta = 0.1;
    float lambda = 0.5;
    float angleStep = 0.196349;
    float halfAngleRange = 0.392699;
...
void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
    ...
    for (int i = 0; i < sectorCount; i++) {
      float angle = float(i) * 6.28318 / float(sectorCount);
...
export interface KuwaharaEffectOptions {
