/**
 * Act 4.1-C — "Industrialization Timelapse, Cell-Shaded"
 *
 * Same timeline and composition as Act 4.1, restyled as a cel-shaded toon
 * using two prebuilt shaders from the three.js ecosystem (no hand-written
 * GLSL in this file):
 *   • THREE.MeshToonMaterial — hard banding via a small gradient texture
 *   • three-stdlib's OutlineEffect — inverted-hull silhouette outlines
 *
 * The ToonSwap component traverses the scene and replaces every
 * MeshStandardMaterial with a paired MeshToonMaterial; per-frame it syncs
 * color/opacity/visible/side from the original so the existing animations
 * (which mutate the std material refs) keep driving the toon look. Emissive
 * MeshBasicMaterial (windows, sun, moon, stars) is left alone so warm
 * pockets and lights still pop.
 */
...
// ─────────────────────────────────────────────────────────────────────
// Library-driven cel shading
//   1. ToonSwap     — swaps MeshStandardMaterial → THREE.MeshToonMaterial
//                     (built-in toon shader from three.js core, hard
//                     banding from a small gradient texture)
//   2. ToonOutlines — three-stdlib's OutlineEffect, an inverted-hull pass
//                     that adds black silhouettes around every mesh
// Neither pass requires hand-written GLSL.
// ─────────────────────────────────────────────────────────────────────
const TOON_BANDS = 4

function makeToonGradient(bands: number) {
  // Stepped DataTexture sampled via NearestFilter → hard cell bands.
  const data = new Uint8Array(bands)
  for (let i = 0; i < bands; i++) {
    data[i] = Math.round(((i + 0.5) / bands) * 255)
  }
...
function ToonSwap({ bands = TOON_BANDS }: { bands?: number }) {
  const scene = useThree((s) => s.scene)
  const gradient = useMemo(() => makeToonGradient(bands), [bands])
  // One toon material per original std material — handles cases where the
  // same std material is shared across multiple meshes.
  const pairs = useRef(new Map<THREE.MeshStandardMaterial, THREE.MeshToonMaterial>())

  useFrame(() => {
    // 1. Discover any unmapped MeshStandardMaterial and swap in a paired toon.
    scene.traverse((obj) => {
    ...
    // 2. Mirror animated props from each original std material onto its
    //    paired toon material so the existing tweens keep driving the look.
...
function ToonOutlines({
  ...
  // Priority > 0 takes over the render loop; R3F skips its default render.
...
// Main export
// ─────────────────────────────────────────────────────────────────────
export default function Act4_1_C() {
  return (
    <div style={{ width: '100vw', height: '100vh', background: '#000' }}>
      ...
      >
        <DebugCamera />
        <SceneContent />
        {/* No EffectComposer here — ToonOutlines owns the render loop via
            OutlineEffect, and the toon look reads flatter without bloom. */}
      </Canvas>
    </div>
  )
}
