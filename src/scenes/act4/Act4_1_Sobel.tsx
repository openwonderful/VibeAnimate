/**
 * Act 4.1-Sobel — "Industrialization Timelapse, Line Art"
 *
 * Same 4.1 scene, post-processed with three-stdlib's SobelOperatorShader
 * (preceded by LuminosityShader for grayscale) to render the scene as
 * pure edge-detected line art on a black background. All shader code is
 * provided by three-stdlib's three.js example ports.
 */
...
// ─────────────────────────────────────────────────────────────────────
// SobelRenderer — three-stdlib pipeline: RenderPass → LuminosityShader
// → SobelOperatorShader. Takes over the render loop via useFrame(_, 1).
// ─────────────────────────────────────────────────────────────────────
function SobelRenderer() {
  ...
  useEffect(() => {
    ...
    const sobel = composer.passes[2] as ShaderPass
    sobel.uniforms.resolution.value.set(size.width * dpr, size.height * dpr)
  }, [size, gl, composer])
...
// Main export
// ─────────────────────────────────────────────────────────────────────
export default function Act4_1_Sobel() {
  return (
