/**
 * Act 4.1-B — "Industrialization Timelapse"
 *
 * Held exterior frame of the hanok (inherits Act 3.2's composition), then
 * the world mutates around the stubborn warm pocket of home:
 *   • Day and night flip on a repeating cycle — stars swap with sun
 *   • Persimmon tree fruits on / off with seasons
 *   • Power lines stitch across the sky
 *   • A smokestack rises from the horizon where mountains used to be
 *   • The neighbor hanok morphs into a concrete box
 *   • A wall of dark city buildings climbs up behind the house
 *   • The dirt road paves itself
 *   • The hanok grows a TV antenna → then a satellite dish, and an AC unit
 *   • Warm amber window glow cools to electric white
 *   • Paddies retreat toward the road
 *   • Chimney smoke thins and stops
 * Settles on the cold pre-dawn blue that opens Act 4.
 */
...
function SkyBodies() {
  ...
  useFrame(({ clock }) => {
    ...
    if (sunRef.current) {
      sunRef.current.position.set(sunX, sunY, -38)
      sunRef.current.visible = sunY > -1.5
    }
    if (moonRef.current) {
      moonRef.current.position.set(moonX, moonY, -38)
...
function CityBuildingHorizon() {
  // Three rows at different depths, each rises from y=-15 at its own threshold.
  const rowA = useMemo<BuildingData[]>(() => generateBuildingRow(5001, 18, 50, 2.5, 6.5).map(b => ({ ...b, baseY: 0 })), [])
  const rowB = useMemo<BuildingData[]>(() => generateBuildingRow(5002, 14, 44, 3.5, 8.5, true).map(b => ({ ...b, baseY: 0 })), [])
  const rowC = useMemo<BuildingData[]>(() => generateBuildingRow(5003, 10, 38, 5.0, 10.0, true).map(b => ({ ...b, baseY: 0 })), [])
  ...
  useFrame(({ clock }) => {
    ...
    if (aRef.current) aRef.current.position.y = lerp(-15, 0, sA)
    if (bRef.current) bRef.current.position.y = lerp(-15, 0, sB)
    if (cRef.current) cRef.current.position.y = lerp(-15, 0, sC)
  })

  return (
    <group>
      <group ref={cRef} position={[0, -15, -48]}>
        {rowC.map((b, i) => <CityBuilding key={i} data={b} color={CITY_DARK_C} />)}
      </group>
      <group ref={bRef} position={[0, -15, -42]}>
        {rowB.map((b, i) => <CityBuilding key={i} data={b} color={CITY_DARK_B} />)}
      </group>
      <group ref={aRef} position={[0, -15, -36]}>
...
// ─────────────────────────────────────────────────────────────────────
// Mountains — same silhouette as Act 3.2, fade out as city rises behind
// ─────────────────────────────────────────────────────────────────────
function FadingMountains() {
  ...
  const farShape = useMemo(() => {
    ...
    shape.moveTo(-30, 0)
    shape.lineTo(-25, 2.5)
    shape.quadraticCurveTo(-22, 4.8, -18, 3.5)
    shape.quadraticCurveTo(-14, 2.2, -10, 5.2)
    shape.quadraticCurveTo(-6, 7.5, -2, 4.0)
    shape.quadraticCurveTo(2, 1.5, 5, 6.0)
    shape.quadraticCurveTo(8, 8.5, 12, 5.5)
    shape.quadraticCurveTo(16, 3.0, 20, 4.5)
    shape.quadraticCurveTo(24, 6.0, 28, 3.0)
    shape.lineTo(30, 0)
    shape.closePath()
    return shape
  }, [])
  const midShape = useMemo(() => {
    ...
    shape.moveTo(-30, 0)
    shape.lineTo(-24, 1.8)
    shape.quadraticCurveTo(-20, 3.5, -15, 2.5)
    shape.quadraticCurveTo(-10, 1.5, -6, 4.0)
    shape.quadraticCurveTo(-2, 5.5, 3, 2.8)
    shape.quadraticCurveTo(7, 1.0, 12, 3.5)
    shape.quadraticCurveTo(17, 5.0, 22, 3.0)
    shape.quadraticCurveTo(26, 1.5, 30, 2.2)
    shape.lineTo(30, 0)
    shape.closePath()
    return shape
  }, [])
  ...
  useFrame(({ clock }) => {
    const { lapse } = computeTimeline(clock.getElapsedTime())
    // Start fading once the first city row is mostly up
    const fade = rampAt(lapse, 0.55, 0.25)
    mat.opacity = 1 - fade
    midMat.opacity = 1 - fade * 0.8
  })

  return (
    <group>
      <mesh position={[0, 0, -35]} rotation={[-Math.PI / 2, 0, 0]} material={mat}>
        <shapeGeometry args={[farShape]} />
      </mesh>
      <mesh position={[0, 0, -25]} rotation={[-Math.PI / 2, 0, 0]} material={midMat}>
...
// Main export
// ─────────────────────────────────────────────────────────────────────
export default function Act4_1_B() {
  return (
