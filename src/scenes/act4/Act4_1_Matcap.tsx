/**
 * Act 4.1-Matcap — "Industrialization Timelapse, Matcap Art"
 *
 * Same 4.1 scene, restyled with THREE.MeshMatcapMaterial. A matcap (material
 * capture) bakes a full lighting setup into a single sphere image; sampling
 * it by the surface normal gives instant art-directed shading with no real
 * light rig. Here the matcap is generated procedurally as a radial gradient
 * so the variant stays self-contained. Swap textures via `makeMatcap` or
 * load any matcap PNG to retarget the whole look.
 */
...
// ─────────────────────────────────────────────────────────────────────
// MatcapSwap — replace every MeshStandardMaterial with a paired
// MeshMatcapMaterial. The matcap texture is a procedural radial gradient
// (warm light → cool shadow + rim) generated once; swap in any matcap PNG
// to retarget the whole scene's look at once.
// ─────────────────────────────────────────────────────────────────────
function makeMatcap(): THREE.DataTexture {
  const size = 256
  const data = new Uint8Array(size * size * 4)
  const cx = size / 2
  const cy = size / 2
  // Color stops driven by distance from sphere center (matcap convention):
  //  center (top-lit) = warm cream, edge (rim) = deep teal, off-sphere = black.
  const center = [255, 232, 195]
  const mid = [180, 140, 120]
  const rim = [40, 80, 110]
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      // Two-stop blend: center → mid (0..0.6), mid → rim (0.6..1.0)
      let cR: number, cG: number, cB: number
      if (r < 0.6) {
        const t = r / 0.6
        cR = center[0] * (1 - t) + mid[0] * t
        cG = center[1] * (1 - t) + mid[1] * t
        cB = center[2] * (1 - t) + mid[2] * t
      } else {
        const t = (r - 0.6) / 0.4
        cR = mid[0] * (1 - t) + rim[0] * t
        cG = mid[1] * (1 - t) + rim[1] * t
        cB = mid[2] * (1 - t) + rim[2] * t
      }
      data[i] = Math.round(cR)
      data[i + 1] = Math.round(cG)
      data[i + 2] = Math.round(cB)
      data[i + 3] = 255
    }
  }
...
function MatcapSwap() {
  ...
  const pairs = useRef(new Map<THREE.MeshStandardMaterial, THREE.MeshMatcapMaterial>())
  ...
  useFrame(() => {
    scene.traverse((obj) => {
      ...
      let mc = pairs.current.get(mat)
      if (!mc) {
        mc = new THREE.MeshMatcapMaterial({
        ...
        pairs.current.set(mat, mc)
      }
      mesh.material = mc
    })
    // Mirror animated props from std → matcap
    pairs.current.forEach((mc, orig) => {
      mc.color.copy(orig.color)
      mc.opacity = orig.opacity
      mc.transparent = orig.transparent
      mc.visible = orig.visible
      mc.side = orig.side
    })
  })
  return null
...
// Main export
// ─────────────────────────────────────────────────────────────────────
export default function Act4_1_Matcap() {
  return (
