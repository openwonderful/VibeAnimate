...
export function generateWorld(seed = 20260725): World {
  ...
  const memberSpots: Placement[] = [
    { x: -3.4, z: -13.5 }, { x: 3.2, z: -12.5 }, { x: -4.8, z: -16.5 },
    { x: 4.6, z: -16 }, { x: -3.0, z: -19 }, { x: 3.8, z: -20 }, { x: 0.4 + roadX(-23) + 2.2, z: -23 },
  ]
  ...
  const mkAscent = (x: number, z: number, named: boolean, member: boolean) => {
    ...
    // Edges lift first, like embers off the rim of a fire.
    let delay = (MAX_R - Math.min(r, MAX_R)) * 0.085 + rand() * 1.7
    let dur = 5.4 + rand() * 2.4
    if (named) { delay = Math.min(delay, 1.1); dur = 4.8 + rand() * 1.0 }
    if (member) { delay = 4.5 + rand() * 0.4; dur = 4.0 + rand() * 0.3 }
    return {
      riseStart: T_ASCEND + delay,
      riseDur: dur,
      apexY: 18 + rand() * 12,
      spiralR: 0.25 + rand() * 0.8,
