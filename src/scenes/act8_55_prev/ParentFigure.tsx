...
import { GroundShadow, lightGain, villageLight } from './locale'
...
export function ParentFigure({ tOff = 0 }: { tOff?: number }) {
  ...
  useFrame(({ camera, size, gl }) => {
    ...
    // 1.9, the same ceiling the hero reaches — they are the same gold from the
    // touch onward. At 1.55 the person he came home to stayed a step dimmer
    // than him for the whole act, which reads as them never quite lighting.
    material.emissiveIntensity = lit * 1.9
    ...
    if (light.current) light.current.intensity = lit * 1.9 * lightGain(1.8)
  })
  ...
  return (
    <group ref={group}>
      ...
      <group ref={body} scale={PARENT_SCALE}>
        {/* The hat they have worn since the road home. Its shell tracks
            `material`, which is dark until he touches them — so the hat comes
            up with the rest of them rather than sitting there lit. */}
        <GoldFigure skeleton={skeleton} material={material} inPlace hat={PARENT_HAT} />
      </group>
      ...
      <pointLight ref={light} color={PARENT_WARM} position={[0, 1.3, 0.3]}
