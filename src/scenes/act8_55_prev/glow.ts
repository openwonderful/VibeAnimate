  ...
  return new THREE.ShaderMaterial({
    ...
    vertexShader: /* glsl */ `
      ...
      void main() {
        ...
        vColor = aColor;
        vAlpha = aAlpha * tw;
        // aSize is a size in the units the points were AUTHORED in, so it has
        // to be scaled by whatever the model matrix does — otherwise mounting
        // this act inside Act B's valley (a group at 20×, see locale.tsx) puts
        // every point 20× further away with the same pixel size, and every
        // halo, lantern and firefly in the scene shrinks to a speck.
        float mscale = length(modelMatrix[0].xyz);
        gl_PointSize = clamp(aSize * mscale * uScale / -mv.z, 0.0, 220.0);
