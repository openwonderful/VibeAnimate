...
// 8.53: like 8.52 but the opening pull-back also RISES and tilts down —
// the frame keeps widening past the field into the far plains and
// mountains as they catch light, drum hit by drum hit.
const KEYS: Key[] = [
  { t: 0.0, pos: [5.4, 1.15, -13.0], tgt: [1.1, 1.0, -20.6], fov: 42 },   // the singer, alone in the dark
  { t: 8.4, pos: [3.3, 3.1, -5.6], tgt: [0.6, 0.8, -23], fov: 51 },       // pulling back + climbing gently
  { t: 16.2, pos: [-2.4, 9.5, 0.5], tgt: [0.2, 0.1, -26], fov: 57 },      // high enough to see the lit horizon
  ...
  // Aim a touch lower than the zenith so the constellations sit high and
  // centered — no barren strip above them.
  { t: 45.2, pos: [3.6, 0.95, -16.7], tgt: [3.2, 38, -40], fov: 70 },     // landing on the road, full look-up
  { t: 48.0, pos: [3.1, 1.15, -17.6], tgt: [4.5, 36.5, -37.5], fov: 72 }, // still drifting as it ends
]
