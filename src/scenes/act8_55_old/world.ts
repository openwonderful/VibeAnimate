...
export function generateWorld(seed = 20260725): World {
  ...
  for (let k = 0; k < ordinary.length; k++) {
    ...
    const takeNamed = namedPool.length > 0 && k % 24 === 12 && !isExtra
    ...
    // A soul can't leave before it catches light. The far field ignites as late
    // as 26.8, so its ascent has to wait — clamped rather than re-derived, so
    // the near field (which lights in the first seconds) keeps 8.53's timing.
