/**
 * OLD 8.55 — the Ascension as it stood before the valley move. FROZEN.
 *
 * Kept as a reference cut, not as a live scene. This is `act8_55/` exactly as it
 * was at commit 824b032, on its OWN invented ground: its own `Terrain` (paddy
 * quilt, snaking road, its own house), its own `SkyDome`, its own ridge
 * silhouette — a careful lookalike for Act B's valley, built before that valley
 * existed. The live 8.55 is now staged in the real one (see `act8_55/locale`),
 * which forced the shot to turn around and look DOWN the valley: the ground
 * behind Act B's house is the pass, and a crowd laid out there stands in a
 * mountain.
 *
 * This copy is deliberately self-contained — every module it needs is beside it,
 * so nothing here is on the live act's import graph and editing the live one
 * cannot quietly change what this renders. It should not be edited; if it stops
 * being useful as a comparison, delete the directory.
 *
 * Viewer: `?act=8.55-old`.
 *
 * ── Original header ──────────────────────────────────────────────────
