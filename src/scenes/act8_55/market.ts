...
/**
 * Road-centre stretch left completely empty for the homecoming: from just short
 * of where he stops, back past the house. This is the quiet pocket 7.2 walks its
 * last five seconds through and the yard 8.55 opens in.
 */
const HOUSE_CLEAR: [number, number] = [-8, -34]
/** The opening camera's tube, as a polyline in the ground plane. */
/**
 * The camera's tube, as a polyline in the ground plane — 8.55-old's restored
 * arc, which crosses the market band rather than stopping short of it. The two
 * numbers below are the whole reason the note "there's a food stall that blocks
 * the view right at the entry" was true: the tube used to describe a camera
 * that sat in the yard, so the first stall beside the opening lens was fair
 * game.
 */
const CAM_TUBE: [number, number][] = [
  [5.4, -13.0], [3.3, -5.6], [-2.4, 0.5], [-1.4, -2.0], [1.2, -6.5],
  [0.4, -11.5], [2.0, -14.0], [3.4, -16.5],
]
const CAM_TUBE_R = 3.0
 ...
 *
 * ── Why the rows are so tight against the road ───────────────────────
 * Act B's road is 62 world units wide with a 30-unit verge either side, and then
 * flooded paddy: in village units that is a road 1.55 across and dry ground out
 * to 2.65. The old rows stood at 2.3–3.65 and a back row at 4.9–6.4, which put
 * every back-row stall — and half the front row — in a bay of `StylizedWater`,
 * selling chestnuts from the middle of a rice paddy. Now the market is one tight
 * aisle on the verge with an occasional stall tucked in behind, which is what a
 * night market on a country road is anyway.
 */
export function generateStalls(seed = 85510): Stall[] {
  ...
  for (let z = 16; z > -13; z -= 1.5 + rand() * 1.0) {
    for (const side of [1, -1]) {
      if (rand() < 0.3) continue                                   // gaps in the row
      if (z < HOUSE_CLEAR[0] && z > HOUSE_CLEAR[1]) continue       // the quiet pocket
      // A back row now and then, so the market has depth off the road — still
      // on the verge, because past 2.65 there is water.
      const back = rand() < 0.26
      const off = back ? 2.45 + rand() * 0.35 : 1.75 + rand() * 0.5
