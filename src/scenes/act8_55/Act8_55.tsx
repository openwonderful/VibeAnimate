...
/** The porch turns down as the pair light — see `porchLight`. */
const houseDim = () => porchLight(getAnimTime())
...
function SceneContent() {
  // The finale's sky is TURNED — see skySpin.tsx for the why and the 176.
  // The spin is applied to a copy of the world here rather than at the
  // source, because `generateWorld()` is also Act B's ascended sky and six
  // other scenes' crowd, and none of them are turning anything.
  const world = useMemo(() => spinWorldSky(generateWorld()), [])
  ...
  return (
    <>
      ...
      <Village>
        ...
        <SpunSky>
          <SkyDome moonCounterSpin={-SKY_SPIN_DEG} />
        </SpunSky>
        ...
        <FarCrowd />
        <Constellations world={world} spun />
      </Village>
    </>
  )
}
