...
/**
 * Count and brightness sized for the landing frame: the final up-shot used to
 * show the risen river in one corner of an otherwise empty black sky — 950
 * stars at alpha 0.22–0.64 across a whole dome leaves a fov-70 slice of it
 * near-blank. These are the ancient stars the souls join, so the sky they
 * arrive in has to already be a sky.
 */
function BackgroundStars({ count = 1700, duskAt, tOff }: { count?: number; duskAt?: (t: number) => number; tOff: number }) {
  ...
  const { positions, colors, sizes, alphas, phases } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      sizes[i] = 0.55 + Math.pow(rand(), 2.2) * 1.8
      alphas[i] = 0.30 + rand() * 0.52
 ...
 */
/**
 * `moonCounterSpin`: degrees to turn the moon BACK when the whole dome is
 * mounted inside a turn (8.55 does — see skySpin.tsx). The dome gradient and
 * the background stars are rotationally symmetric and do not care; the nebula
 * bed has to follow the river and must not be excluded. The moon is the one
 * piece that is placed for the FRAME rather than for the sky: at az 46 it
 * sits comfortably out of shot, and carried 176° round with everything else
 * it lands in the bottom-right corner of the last frame of the film.
 */
export function SkyDome({
  ...
  timeOffset?: number
  duskAt?: (t: number) => number
  moonCounterSpin?: number
} = {}) {
  return (
    <group>
