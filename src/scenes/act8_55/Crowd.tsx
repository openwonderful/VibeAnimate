...
import { GroundShadow, groundY, lightGain, villageLight } from './locale'
...
export function CrowdSouls({ world }: { world: World }) {
  ...
  useFrame(({ camera, size, gl }) => {
    ...
    // The window's FIRST TWO beats (18.06, 18.82) land while line 3's own
    // 1.4s swell is still near the floor, and a fixed 0.38× multiplier on
    // that small baseline was invisible — the note came back "we're missing
    // the first two beats right off the bat". The gain leans against the
    // phrase attack so the flash's ABSOLUTE brightness is roughly constant
    // across the whole window: ~1.0× while the baseline is low, settling to
    // 0.38× once the swell holds.
    const pulseGain = 0.38 * (1 + 1.7 * (1 - phrase))
    ...
    for (let i = 0; i < N; i++) {
      ...
      gAlpha *= 1 + pulseGain * pulse
      sSize *= 1 + 0.08 * (1 + 1.2 * (1 - phrase)) * pulse
...
export function GoldenFigure() {
  ...
  useFrame(({ camera, size, gl }) => {
    ...
    // halo out so it doesn't wash the final frame. (42.0: the landing runs
    // ~1.6s earlier now — see CameraRig's last keys.)
    const near = 1 - smooth01((t - 42.0) / 1.6)
    ...
    if (light.current) {
      light.current.intensity =
        ((2.4 + phrase * 2.0) * (0.5 + 0.5 * near) * lit + flash * 5) * lightGain(1.8)
    }
  })

  // Turned to face the house he is singing to — and then a third of a radian
  // off it, toward the person standing at his shoulder (they stand on his +x
  // side now, by the porch, so the turn goes the other way than it used to).
  // Square to the house he is a back to the lens; at this angle the opening
  // frame gets his profile, their profile, and the gap between the two hands.
  return (
    <group position={GOLDEN_POS} rotation={[0, Math.PI - 0.34, 0]}>
      <GroundShadow />
