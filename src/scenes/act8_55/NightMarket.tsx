...
import { villageLight } from './locale'
import {
...
function Banner({ tOff }: { tOff: number }) {
  ...
  return (
    <group position={[x, 0, BANNER_Z]}>
      ...
      {/* Printed BOTH sides, as two back-to-back quads rather than one
          `DoubleSide` plane. It used to be the latter, which was fine while
          only one act looked at it: a `DoubleSide` plane seen from behind
          renders 야시장 back to front, and this banner is now read from both
          ends of the road — 7.2 trails him up to it and sees the down-valley
          face, 8.55 stands at the house and sees the other one. A festival
          banner is printed on both sides anyway.
          The slight sag is one rotation on each; two quads is also what makes
          the cloth read as cloth rather than as a decal. */}
      {[0, Math.PI].map(yaw => (
        <mesh key={yaw} position={[0, BANNER_Y, 0]} rotation={[0.05, yaw, 0]}>
          <planeGeometry args={[halfSpan * 2, 1.16]} />
          <meshStandardMaterial
            ref={yaw === 0 ? cloth : undefined}
            map={texture} emissiveMap={texture} emissive="#FFFFFF"
            emissiveIntensity={0.85} roughness={1}
          />
        </mesh>
      ))}
      <pointLight position={[0, BANNER_Y - 0.9, 0.5]} color={LANTERN_COLOR}
        {...villageLight(2.4, 9, 1.7)} />
    </group>
  )
}
