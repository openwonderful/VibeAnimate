/**
 * CharStickGold3 — Stick Gold with slightly shorter legs.
 *
 * Clone of CharStickGold2 with the leg segment shortened by ~14%
 * (hips lowered from 0.92 → 0.80) and torso/head/arms shifted down to match.
 * Head remains the same size; everything above the hips keeps its proportions.
 * Purpose: isolate the "shorter legs" look from camera angle.
 */
...
function SmoothFigure() {
  ...
  // Shortened-leg pose. Hips lowered from 0.92 → 0.80. Torso/head/arms
  // shift down by 0.12 to keep upper body proportions. Feet stay at y=0.06.
  const bodyGeo = useSmoothBody([
    // Spine: hips → shoulders → neck
    {
      points: [
        new THREE.Vector3(0, 0.78, 0),
        new THREE.Vector3(0.01, 0.93, 0.02),
        new THREE.Vector3(0.01, 1.08, 0.03),
        new THREE.Vector3(0, 1.28, 0.01),
        new THREE.Vector3(0, 1.42, 0.01),
      ],
      radius: R,
      segments: 24,
    },
    ...
    {
      points: [
        new THREE.Vector3(0, 1.26, 0.01),
        new THREE.Vector3(-0.15, 1.28, 0.04),
        new THREE.Vector3(-0.30, 1.38, 0.06),
        new THREE.Vector3(-0.40, 1.53, 0.03),
      ],
      radius: R,
      segments: 16,
    },
    // Right arm: swept back
    {
      points: [
        new THREE.Vector3(0, 1.26, 0.01),
        new THREE.Vector3(0.15, 1.20, -0.03),
        new THREE.Vector3(0.28, 1.06, -0.06),
        new THREE.Vector3(0.35, 0.90, -0.08),
      ],
      radius: R,
      segments: 16,
    },
    // Left leg: stepping forward (shortened — ~14% less)
    {
      points: [
        new THREE.Vector3(0, 0.80, 0),
        new THREE.Vector3(-0.08, 0.59, 0.12),
        new THREE.Vector3(-0.14, 0.34, 0.18),
        new THREE.Vector3(-0.16, 0.06, 0.12),
      ],
      radius: R,
      segments: 18,
    },
    // Right leg: planted behind (shortened)
    {
      points: [
        new THREE.Vector3(0, 0.80, 0),
        new THREE.Vector3(0.06, 0.56, -0.08),
        new THREE.Vector3(0.10, 0.30, -0.10),
    ...
    { center: new THREE.Vector3(0, 1.42, 0.01), radius: RH },
  ])
...
function SceneSetup() {
  ...
  useEffect(() => { camera.lookAt(0, 0.78, 0) }, [camera])
  return null
}

export default function CharStickGold3() {
  return (
    <div style={{ width: '100%', height: '100%', background: BG }}>
      <Canvas
        ...
        camera={{ position: [0, 0.78, 2.4], fov: 45, near: 0.1, far: 100 }}
      >
        <SceneSetup />
        <DebugCamera />
