/**
 * Act 3.1 — "The Road Home" (SPIDER-VERSE STYLE)
 *
 * Same sunset scene — parent carrying child down a dirt road through rice
 * paddies — but rendered with a Spider-Verse-inspired comic book shader:
 * bold ink outlines, halftone Ben-Day dots, hard posterization,
 * chromatic aberration, high contrast. No depth-of-field blur.
 */
...
/* ─── Spider-Verse-style post-processing effect ────────────────── */
/*
 * Spider-Verse is 90% clean render + sparse comic accents.
 * Bold outlines on silhouettes, subtle color fringing,
 * small halftone dots ONLY on bright specular/rim areas.
 * The image stays beautiful and readable.
 */
const spiderVerseFragmentShader = /* glsl */ `
  void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
    vec2 texelSize = 1.0 / resolution;

    // ── 1. Subtle Plate Misregistration ──
    vec3 col;
    col.r = texture2D(inputBuffer, uv + vec2(-0.0008, 0.0003)).r;
    col.g = inputColor.g;
    col.b = texture2D(inputBuffer, uv + vec2(0.0005, -0.0004)).b;

    float lum = dot(col, vec3(0.299, 0.587, 0.114));

    // ── 2. Bold Outlines (only strong silhouettes) ──
    float edgeSum = 0.0;
    for (float rad = 2.0; rad <= 6.0; rad += 2.0) {
      vec2 o = texelSize * rad;
      float tl = dot(texture2D(inputBuffer, uv + vec2(-o.x, -o.y)).rgb, vec3(0.3, 0.59, 0.11));
      float tc = dot(texture2D(inputBuffer, uv + vec2( 0.0, -o.y)).rgb, vec3(0.3, 0.59, 0.11));
      float tr = dot(texture2D(inputBuffer, uv + vec2( o.x, -o.y)).rgb, vec3(0.3, 0.59, 0.11));
      float ml = dot(texture2D(inputBuffer, uv + vec2(-o.x,  0.0)).rgb, vec3(0.3, 0.59, 0.11));
      float mr = dot(texture2D(inputBuffer, uv + vec2( o.x,  0.0)).rgb, vec3(0.3, 0.59, 0.11));
      float bl = dot(texture2D(inputBuffer, uv + vec2(-o.x,  o.y)).rgb, vec3(0.3, 0.59, 0.11));
      float bc = dot(texture2D(inputBuffer, uv + vec2( 0.0,  o.y)).rgb, vec3(0.3, 0.59, 0.11));
      float br = dot(texture2D(inputBuffer, uv + vec2( o.x,  o.y)).rgb, vec3(0.3, 0.59, 0.11));
    ...
    edgeSum /= 3.0;
    float outlineMask = smoothstep(0.12, 0.25, edgeSum);
    col = mix(col, vec3(0.01), outlineMask * 0.9);

    // ── 3. Sparse CMYK Halftone — ONLY on bright specular/rim areas ──
    // Spider-Verse uses dots selectively on highlights, not everywhere.
    float specularMask = smoothstep(0.7, 0.95, lum);
    ...
    if (specularMask > 0.01) {
      ...
      float freq = 50.0;
      vec2 st = uv * vec2(resolution.x / resolution.y, 1.0);

      vec2 Cc = 2.0 * fract(freq * mat2(0.966, -0.259, 0.259, 0.966) * st) - 1.0;
      float cDot = smoothstep(-0.04, 0.04, sqrt(cmyk.x) - length(Cc));

      vec2 Mc = 2.0 * fract(freq * mat2(0.966, 0.259, -0.259, 0.966) * st) - 1.0;
      float mDot = smoothstep(-0.04, 0.04, sqrt(cmyk.y) - length(Mc));

      vec2 Yc = 2.0 * fract(freq * st) - 1.0;
      float yDot = smoothstep(-0.04, 0.04, sqrt(cmyk.z) - length(Yc));

      vec3 halftone = 1.0 - 0.7 * vec3(cDot, mDot, yDot);
      col = mix(col, halftone, specularMask * 0.5);
    }

    // ── 4. Panel Border ──
    float bw = 0.004;
    float bMask = smoothstep(0.0, bw, uv.x) * smoothstep(0.0, bw, 1.0 - uv.x)
               * smoothstep(0.0, bw, uv.y) * smoothstep(0.0, bw, 1.0 - uv.y);
    col *= bMask;
...
class SpiderVerseEffect extends Effect {
  constructor() {
    super('SpiderVerseEffect', spiderVerseFragmentShader, {
      blendFunction: BlendFunction.NORMAL,
    })
  }
}

const SpiderVerseStyle = forwardRef<SpiderVerseEffect>(function SpiderVerseStyle(_props, ref) {
  const effect = useMemo(() => new SpiderVerseEffect(), [])
...
/* ─── Main export ───────────────────────────────────────────────── */
export default function Act3_1_SpiderVerse() {
  useEffect(() => {
