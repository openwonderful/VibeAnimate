...
import {
  BEAT_HITS, FIREFLY_GREEN, GOLD, GOLD_BRIGHT, GOLDEN_POS, phraseEnv, stillness, T_DROP,
} from './constants'
...
export function CrowdSouls({ world }: { world: World }) {
  ...
  const statics = useMemo(() => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      ignite[i] = snapped + ((i * 37) % 11) / 11 * 0.12
    }
    return {
...
function GoldenFigure() {
  ...
  useFrame(({ camera, size, gl }) => {
    ...
    if (alphaRef.current) {
      ...
      arr[0] = (0.55 + phrase * 0.3) * near
      arr[1] = (0.15 + phrase * 0.1) * near * far
