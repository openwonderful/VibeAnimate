...
import {
  ...
  HEAVY_HITS, HOUSE_Z, roadX, SKY_R, STAR_BLUE, STAR_PURPLE, STAR_WARM,
} from './constants'
...
export type Figure = {
  ...
  /** 8.54: wakes on this heavy beat instead of the radial drum-ring spread */
  igniteOverride?: number
}
...
export function generateWorld(seed = 20260725): World {
  ...
  for (let k = 0; k < ordinary.length; k++) {
    ...
    // 8.54: a quarter of the crowd stays invisible through the intro and
    // WAKES on the heavier beats from 18s — the world keeps growing.
    if (!named && rand() < 0.24) {
      const hit = HEAVY_HITS[Math.floor(rand() * HEAVY_HITS.length)]
      fig.igniteOverride = hit
      fig.riseStart = Math.min(hit + 2.2 + rand() * 4.5, 31)
      fig.riseDur = 10.5 + rand() * 3
    }
    figures.push(fig)
