...
import {
  beatPulse, BEAT_HITS, FIREFLY_GREEN, GOLD, GOLD_BRIGHT, GOLDEN_POS, phraseEnv, stillness, T_DROP,
} from './constants'
...
export function CrowdSouls({ world }: { world: World }) {
  ...
  const statics = useMemo(() => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      // The song spreads outward from the singer on the road: light catches
      // each figure by distance, with human raggedness.
      const dGold = Math.hypot(f.x - GOLDEN_POS[0], f.z - GOLDEN_POS[2])
      // Radial spread FROM the singer/house outward — the first drum hit
      // (2.06s) lights the ring of people right around the house, in the
      // center of the opening frame; each following hit reaches further out.
      // Area-weighted radius→time curve: inner rings are small, so a linear
      // map gives the first drum hits almost nobody. The ^1.5 curve hands
      // each hit a similar-sized crowd, starting with the ring around the
      // house that the opening camera is actually looking at. The extended
      // outer field (dGold > 34) fills in on the last four hits.
      const raw = dGold <= 34
        ? 0.9 + 15.9 * Math.pow(dGold / 34, 1.5) + ((i * 73) % 29) / 29 * 0.4
        : 14.06 + ((dGold - 34) / 28) * 2.7 + ((i * 73) % 29) / 29 * 0.3
      let snapped = BEAT_HITS[BEAT_HITS.length - 1] + 0.3
      for (const h of BEAT_HITS) {
        if (h >= raw) { snapped = h; break }
      }
      // Late bloomers wake on their assigned heavy beat instead.
      ignite[i] = (f.igniteOverride ?? snapped) + ((i * 37) % 11) / 11 * 0.12
    }
    return {
      bodyColors, palette, soulColors, glowPositions, glowColors, glowSizes, glowAlphas,
      glowPhases, jumpDelay, ignite, groundCol, targetCol,
    }
  }, [figures, N])
  ...
  useFrame(({ camera, size, gl }) => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const igFlash = t > ignite[i] ? Math.exp(-(t - ignite[i]) * 2.4) * 0.7 : 0
      ...
      const lit = 0.025 + 0.68 * igK + igFlash * 0.3
      ...
      if (k <= 0) {
        ...
        gAlpha = ((0.34 + 0.38 * phrase + breathGlow + dropFlash) * vary * igK * (0.78 + 0.5 * sing) + igFlash)
          * (f.isMember ? 1.6 : 1)
...
function Fireflies({ count = 170 }: { count?: number }) {
...
function GoldenFigure() {
  ...
  useFrame(({ camera, size, gl }) => {
    ...
    if (alphaRef.current) {
      ...
      // The singer burns bright from frame one — the source of the ignition.
      arr[0] = (0.55 + phrase * 0.3 + pulse * 0.22) * near
      arr[1] = (0.15 + phrase * 0.1 + pulse * 0.06) * near * far
      alphaRef.current.needsUpdate = true
    }
    if (light.current) light.current.intensity = (2.4 + phrase * 2.0) * (0.5 + 0.5 * near)
  })
