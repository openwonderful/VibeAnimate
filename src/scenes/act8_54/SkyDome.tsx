...
function Dome() {
  const material = useMemo(() => new THREE.ShaderMaterial({
    uniforms: {
      ...
      uHorizon: { value: new THREE.Color('#0B1526') },
      uHaze: { value: new THREE.Color('#16233C') },
    },
    vertexShader: /* glsl */ `
      varying float vY;
      void main() {
...
function BackgroundStars({ count = 950 }: { count?: number }) {
  ...
  useFrame(({ camera, size, gl }) => {
    updateGlowScale(material, camera, size.height * gl.getPixelRatio(), getAnimTime())
  })
...
function Moon() {
  ...
  useFrame(({ camera, size, gl }) => {
    updateGlowScale(material, camera, size.height * gl.getPixelRatio(), getAnimTime() * 0.12)
  })
  ...
  return (
    <group position={base}>
      ...
      <mesh>
        ...
        <meshBasicMaterial
          color="#E8DCB8" transparent opacity={0.16} depthWrite={false}
...
function NebulaBand({ count = 130 }: { count?: number }) {
  const material = useMemo(() => makeGlowMaterial({ depthTest: true }), [])
