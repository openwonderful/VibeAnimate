...
import {
  HOUSE_GLOW, HOUSE_Z, MOON_COLOR, PARENT_WARM, phraseEnv, roadX, T_DROP,
} from './constants'
...
function Paddies({ parcels }: { parcels: Parcel[] }) {
  ...
  useFrame(() => {
    if (waterMat.current) {
      waterMat.current.emissiveIntensity = 0.32 * earthDim(getAnimTime())
    }
  })
...
function Farmhouse() {
  ...
  const parentGlow = useRef<THREE.Mesh>(null)
  ...
  useFrame(() => {
    ...
    if (parentGlow.current) {
      const m = parentGlow.current.material as THREE.MeshBasicMaterial
      m.opacity = 0.16 + Math.sin(t * 0.7) * 0.05
      parentGlow.current.scale.setScalar(1 + Math.sin(t * 0.8) * 0.07)
    }
  })
  ...
  return (
    <group position={[0, 0, HOUSE_Z]}>
      ...
      {/* The parent — one small warm silhouette in the doorway, watching */}
      <group position={[0.14, 0, wallD / 2 + 0.1]}>
        <mesh position={[0, 0.42, 0]}>
          <capsuleGeometry args={[0.09, 0.5, 4, 8]} />
          <meshStandardMaterial color="#1A130C" emissive={PARENT_WARM} emissiveIntensity={0.55} roughness={0.8} />
        </mesh>
        <mesh position={[0, 0.78, 0]}>
          <sphereGeometry args={[0.075, 8, 8]} />
          <meshStandardMaterial color="#1A130C" emissive={PARENT_WARM} emissiveIntensity={0.55} roughness={0.8} />
        </mesh>
        <mesh ref={parentGlow} position={[0, 0.55, 0.05]}>
          <sphereGeometry args={[0.28, 10, 10]} />
          <meshBasicMaterial
            color={PARENT_WARM} transparent opacity={0.16} depthWrite={false}
...
function HouseGlowSprites({ wallD }: { wallD: number }) {
  ...
  useFrame(({ camera, size, gl }) => {
    updateGlowScale(material, camera, size.height * gl.getPixelRatio(), getAnimTime() * 0.4)
    ...
    setGlowGlobal(material, earthDim(getAnimTime()) * smooth01((d - 11) / 13))
  })
...
export function Terrain({ parcels }: { parcels: Parcel[] }) {
  return (
    <group>
      {/* Base earth */}
